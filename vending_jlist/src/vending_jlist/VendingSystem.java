...
public class VendingSystem implements ActionListener {
	...
	private static ArrayList<VendingItem> current_items = new ArrayList<VendingItem>();
	...
	public void setupGUI() {
		...
		for (int i = 0; i < current_items.size(); i++) {
			model.addElement(current_items.get(i));
		}
	...
	private static void createList() {
		current_items.add(chocolate);
		current_items.add(beer);
		current_items.add(lemonade);
		current_items.add(cola);
		current_items.add(crisps);
		current_items.add(curryPot);
	}
}
